...
class _SyncPhase:
  ...
  def block_on(self, tree):
    ...
    try:
      ...
      logging.warning(
          "Could not block on '%s' outputs (%s); its time excludes any work "
          "still in flight.", self.label, exc,
      )
  ...
  def __exit__(self, exc_type, exc_value, traceback):
    if exc_type is not None:
      ...
      print(
          f"[weight-sync] {self.label}: FAILED after "
          f"{time.perf_counter() - self._start:.4f} s (no measurement)",
          flush=True,
      )
      return False
    ...
    if self._before and after:
      ...
      delta = max(
          after[d][0] - self._before.get(d, (0, 0))[0] for d in after
      )
      line += (
          f" | HBM in_use {_gib(in_use):.2f} GiB "
          f"(delta {_gib(delta):+.2f}) | peak {_gib(peak):.2f} GiB"
      )
    else:
...
def _reshard_and_assign_converted(maxtext_vllm_state, golden_llm_state, llm):
  ...
  """
  from flax import traverse_util  # pylint: disable=import-outside-toplevel
  from tunix.generate import utils as gen_utils  # pylint: disable=import-outside-toplevel
  ...
  resharded_flat = gen_utils._reshard_in_chunks(  # pylint: disable=protected-access
      src_flat,
      spec_flat,
      reshard_pytree,
      chunk_size=128,
  ...
  if isinstance(golden_llm_state, nnx.State):
    ...
    llm.llm_engine.model_executor.driver_worker.model_runner.model.state = (
        resharded_weights
    )
...
def _build_weight_converter(model_name: str, direct: bool, config, tp: int) -> WeightConverter:
  ...
  rules = MODEL_TO_CONVERSION_RULES.get(
      model_name, MODEL_TO_CONVERSION_RULES["qwen3_moe"]
  )
  if rules is None:
    raise ValueError(
