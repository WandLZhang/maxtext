...
class MaxTextVllmSampler(VllmSampler):
  ...
  def update_params(
    ...
    try:
      ...
      logging.error(
          "MaxTextVllmSampler.update_params failed:\n%s", traceback.format_exc()
      )
...
class MaxTextVllmRollout(vllm_rollout.VllmRollout):
  ...
  def __init__(
    ...
    self._weight_sync_debug = bool(
        getattr(maxtext_config, "weight_sync_debug", False)
        or vllm_additional_config.get("weight_sync_debug", False)
    )
  ...
  def update_params(
    ...
    if getattr(self._maxtext_config, "log_weight_sync_norm", False):
      ...
      if param_leaves:
        l2_norm = float(
            jnp.sqrt(sum(jnp.sum(jnp.square(p.astype(jnp.float32))) for p in param_leaves))
        )
  ...
  def _timed_weight_sync(self, label: str):
    ...
    try:
      ...
      logging.info(
          ...
          index, label, time.perf_counter() - start,
      )
      raise
    logging.info(
        "weight_sync_time: sync %d (%s) via %s: %.4f s",
        index, label, self._sync_path_name(), time.perf_counter() - start,
    )
  ...
  def _block_on_rollout_state(self, index: int) -> None:
    ...
    try:
      jax.block_until_ready(
          jax.tree_util.tree_leaves(self._sampler.transformer_state)
      )
    except Exception as exc:  # pylint: disable=broad-except
      logging.warning(
          "weight_sync_time: could not block on rollout state (%s); sync %d "
          "excludes any work still in flight.", exc, index,
      )
