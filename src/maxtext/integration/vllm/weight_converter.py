...
class WeightConverter:
    ...
    def __init__(
        ...
        if rules is None:
            ...
            self._direct = MaxTextToMaxTextConverter(
                config=config,
                moe_fused_layout=(
                    moe_fused_layout or MoEFusedLayout.PER_SHARD_INTERLEAVE
                ),
                allow_unused_source_keys=allow_unused_source_keys,
                debug=debug,
            )
            logging.info(
                "WeightConverter: direct MaxText-to-MaxText mode (debug=%s).", debug
            )
        else:
            logging.info(
    ...
    def convert(
        ...
        flat_src = traverse_util.flatten_dict(_to_pure(src_pytree), sep='.')
        gc.collect()
...
def _group_plan(plan: List[_PlanEntry]) -> List[_PlanGroup]:
    ...
    *not* part of the key: every rollout layer reading a given scanned source
    ...
    check catches loudly rather than silently mis-writing.
    """
...
def _is_non_weight_path(key_tuple: Tuple[Any, ...]) -> bool:
    return any(
        isinstance(part, str) and part.lstrip("_").startswith(_NON_WEIGHT_PATH_PREFIXES)
...
def _non_param_paths(state: Any) -> frozenset:
    ...
    try:
        ...
        logging.debug("Could not inspect nnx variable types; using name filter.")
        return frozenset()
...
class MaxTextToMaxTextConverter:
    ...
    def __init__(
        ...
        logging.info(
            ...
            "scan_axis=%d, moe_fused_layout=%s",
            self.num_decoder_layers, self.cycle, self.num_blocks,
            self.scan_axis, self.moe_fused_layout,
        )
    ...
    def _validate_plan(self, src_flat, tgt_flat, unmatched, consumed) -> None:
        ...
        unused = [
            k for k in src_flat
            if k not in consumed and not _is_non_weight_path(k)
        ...
        logging.info(
            "MaxTextToMaxTextConverter: plan covers %d rollout parameters from "
            "%d trainer parameters.", len(tgt_flat), len(consumed),
        )
    ...
    def _fuse_moe_bulk(self, wi_0, wi_1, tgt_val, key_path: str):
        ...
        scan_fused_axis = (
            tgt_fused_axis
            if tgt_fused_axis < self.scan_axis
            else tgt_fused_axis + 1
        )
        ...
        if self.moe_fused_layout == MoEFusedLayout.CONCAT:
            ...
            half_shape = (
                tgt_shape[:tgt_fused_axis] + (half,) + tgt_shape[tgt_fused_axis + 1:]
            )
            scanned_half_shape = (
                half_shape[:self.scan_axis]
                + (wi_0.shape[self.scan_axis],)
                + half_shape[self.scan_axis:]
            )
            scanned_sharding = _scanned_sharding_from_per_layer(
                getattr(tgt_val, "sharding", None), self.scan_axis
            )
            padded = [
                _align_per_axis(w, scanned_half_shape, scanned_sharding, key_path)
                for w in (wi_0, wi_1)
            ]
    ...
    def _execute_group(self, group: _PlanGroup, src_flat, tgt_flat):
        ...
        if group.op == "identity":
            val = _apply_dtype_cast(
                src_flat[group.source_keys[0]], first_tgt.dtype, path
            )
            out = _align_per_axis(
                val, first_tgt.shape, getattr(first_tgt, "sharding", None), path
            )
        ...
        if group.op == "fuse_moe":
            wi_0, wi_1 = (
                _apply_dtype_cast(src_flat[k], first_tgt.dtype, path)
            ...
            per_block = _bulk_align_and_unstack(
                val, self.scan_axis, first_tgt, path
            )
    ...
    def convert(self, src_pytree: Any, target_state: Any = None) -> Dict[str, Any]:
        ...
        if tgt_root:
            ...
            skip_paths = frozenset(
                p[depth:] for p in skip_paths if p[:depth] == tgt_root
            )
        ...
        if self.debug:
            ...
            logging.info(
                ...
                len(self._plan), len(self._groups), src_span, tgt_span,
            )
        ...
        for group in self._groups:
            if self.debug:
                for k in group.source_keys:
                    logging.info(
                        "weight_sync_debug: op=%s source=%s (%d targets) | src %s "
                        "| tgt %s",
                        group.op, ".".join(map(str, k)), len(group.targets),
                ...
                logging.info(
                    ...
                    group.op, group.source_path, len(outs),
...
def _rekey_to_target(flat_dotted: Dict[str, Any], target_state: Any) -> Dict[str, Any]:
    ...
    if unmatched:
        logging.warning(
            "%d converted tensors have no counterpart in the rollout state and "
            "were dropped, e.g. %s",
...
def _strip_root(
    flat: Mapping[Tuple[Any, ...], Any], root: str
) -> Tuple[Dict[Tuple[Any, ...], Any], Tuple[Any, ...]]:
