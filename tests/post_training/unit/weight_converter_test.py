...
def _arr(*shape, offset=0):
  ...
  return jnp.asarray(
      (np.arange(size, dtype=np.float32) + float(offset)).reshape(shape)
  )
...
def _source_tree(fused_moe_on_target: bool):
  ...
  for slot in range(CYCLE):
    ...
    # a mis-mapped slot or a swapped wi_0/wi_1 shows up as a value mismatch
    ...
    if slot == CYCLE - 1:
      block["self_attention"] = {
          "query": {"kernel": _scanned(EMB, 2, 4, offset=base + 500)}
      }
    else:
      block["linear_attention"] = {
          "conv": {"kernel": _scanned(EMB, 1, 4, offset=base + 500)}
      }
...
class ScannedToUnrolledMappingTest(unittest.TestCase):
  ...
  def test_hybrid_layer_types_both_transfer(self):
    ...
    out = MaxTextToMaxTextConverter(_config()).convert(
        _source_tree(True), _target_tree()
    )["decoder"]
...
class MoEFusionTest(unittest.TestCase):
  ...
  def test_padded_target_intermediate_is_honoured(self):
    ...
    padded = 16
    out = MaxTextToMaxTextConverter(_config()).convert(
        _source_tree(True), _target_tree(moe_intermediate=padded)
    )
    self.assertEqual(
  ...
  def test_concat_layout_places_gate_before_up(self):
    converter = MaxTextToMaxTextConverter(
        _config(), moe_fused_layout=MoEFusedLayout.CONCAT
    )
    out = converter.convert(_source_tree(True), _target_tree())
  ...
  def test_padded_wo_is_zero_padded_not_repeated(self):
    ...
    padded = 16
    out = MaxTextToMaxTextConverter(_config()).convert(
        _source_tree(True), _target_tree(wo_intermediate=padded)
    )
    ...
    np.testing.assert_array_equal(
        wo[:, MOE_DIM:, :], np.zeros((EXPERTS, padded - MOE_DIM, EMB))
    )

  def test_unfused_target_takes_wi_0_and_wi_1_directly(self):
    out = MaxTextToMaxTextConverter(_config()).convert(
        _source_tree(False), _target_tree(fused=False)
    )
...
class CoverageValidationTest(unittest.TestCase):
  ...
  def test_allowlisted_source_param_is_tolerated(self):
    ...
    converter = MaxTextToMaxTextConverter(
        _config(), allow_unused_source_keys=("vision_tower",)
    )
...
class WeightConverterModeDispatchTest(unittest.TestCase):
  ...
  def test_direct_only_models_have_no_hf_rule_table(self):
    ...
    self.assertIsNone(_MODEL_TO_CONVERSION_RULES["qwen35_moe"])
    self.assertTrue(_MODEL_TO_CONVERSION_RULES["qwen3_moe"])


def _mesh(devices):
  return jax.sharding.Mesh(
      np.asarray(devices).reshape(2, 2), ("fsdp", "tensor")
  )
...
def _place(tree, mesh, spec_for):
  ...
  return traverse_util.unflatten_dict({
      key: jax.device_put(
          value, jax.sharding.NamedSharding(mesh, spec_for(key, value))
      )
...
class CrossMeshPlacementTest(unittest.TestCase):
  ...
  *after* conversion. If any converter op inherits its device assignment from
  ...
  def _converted(self, moe_intermediate=MOE_DIM, target_mesh=None):
    source = _place(
        _source_tree(True), self.src_mesh, lambda *_: jax.sharding.PartitionSpec()
    )
    target = _place(
  ...
  def test_no_converted_weight_lands_on_the_target_mesh(self):
    ...
    self.assertEqual(
        leaked, {}, f"converted weights placed on rollout-mesh devices: {leaked}"
    )
  ...
  def test_splitting_the_meshes_does_not_change_the_values(self):
    ...
    for key, want in traverse_util.flatten_dict(same_mesh).items():
      np.testing.assert_array_equal(
          np.asarray(cross_flat[key]), np.asarray(want),
